...
class BaseSolutionIdea(BaseModel):
    ...
    idea_id: Optional[str] = Field(
        default=None,
        description="CODE-FILLED durable candidate identity; never generated by the LLM",
    )
    idea_revision: int = Field(
        default=1,
        ge=1,
        description="CODE-FILLED immutable candidate revision selected by exact references",
    )
    identity_origin: Optional[str] = Field(
        default=None,
        description="CODE-FILLED identity namespace: phase1, regeneration, seed, or legacy_backfill",
    )
    identity_operation_id: Optional[str] = Field(
        default=None,
        description="CODE-FILLED operation key used to mint the durable candidate identity",
    )
    ...
    # Exact Concept Forge evaluation provenance. Unlike a free-text user_seed, this
    # identifies the immutable selected option and carries its complete decision brief
    # through checkpoint and preview serialization, including demoted outcomes.
    evaluation_id: Optional[str] = Field(
        default=None,
        description="CODE-FILLED dispatch-backed identity for an exact concept evaluation",
    )
    evaluation_source_message_id: Optional[str] = Field(
        default=None,
        description="CODE-FILLED proposal message identity for an exact concept evaluation",
    )
    proposed_title: Optional[str] = Field(
        default=None,
        description="CODE-FILLED immutable title selected in Concept Forge",
    )
    ...
    data_access_model: Optional[str] = Field(
        default=None,
        description=(
            "How the required data is obtained: 'public' (incl. no external data at all — pure "
            "computation / user-supplied input) | 'freemium' | 'paywalled' | "
            "'unofficial' (unofficial API / scraping lib, ToS-gray — kept + flagged) | "
            "'restricted' | 'blocked' | 'unverified'. The Stage-7 critic drops 'blocked' RawConcepts, but the v4 "
...
class RawConcept(BaseModel):
    ...
    data_access_model: Optional[str] = Field(
        ...
        description=("public | freemium | paywalled | unofficial | restricted | blocked | "
                     "unverified. 'public' also covers needing no external data at all."),
    )
    data_acquisition_notes: Optional[str] = Field(
        default=None,
