// The empty-parity phrasing is INTERPOLATED from the vocabulary helper, never retyped here.
// This entry is a few-shot: it tells the analyst to quote the wording it was handed, so a
// literal copy of the old claim ("No competing product found") would have re-taught the model
// the exact sentence the helper stopped emitting, and the helper's fix would have been
// defeated by its own documentation.
import { NONE_SURFACED_PHRASE } from '../utils/selectionVocabulary.js';
...
const METRIC_EXPLANATIONS: Record<string, string> = {
  ...
  incumbent_parity: `The web-verified finding on whether an existing product already ships this idea’s core mechanism. Every tool result already presents it in product wording — "Already shipped by X", "Partly covered by X", "Buyers already get this outcome from X", "Already included free with X", each followed by its evidence — so quote the phrasing you were given rather than any shorthand for it. When the search came back empty the tool result says so in the SAME wording every time: "${NONE_SURFACED_PHRASE}" That empty result is a fact about that search and never proof that no competitor exists — the queries are built from the idea’s own vocabulary, so a rival that describes itself differently is missed, and about nine in ten runs that produce an empty result for one idea name an incumbent on another idea in the same run. Never restate it as no competition, an open lane, an unserved market, a first-mover position, or whitespace, and never treat it as a point in the idea’s favour. A finding requires a NAMED vendor and evidence from a search run for that specific idea; with no named vendor there is no parity finding at all. Since 2026-08-02 the adversarial review no longer writes into this channel, so its objections appear as review caveats and never borrow a competitor’s authority. Findings cap scores downward only. Sources: _probe_mechanism_parity and _validate_idea_caps.`,
