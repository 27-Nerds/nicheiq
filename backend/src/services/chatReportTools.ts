...
const METRIC_EXPLANATIONS: Record<string, string> = {
  ...
  red_team_verdict: 'The adversarial review’s verdict on an idea’s PREMISE — usually that a reachable buyer wants this at all — searched for from evidence. Every tool result already presents it in product wording, one of "Premise unproven", "a decision-critical objection", or "no disqualifying objection"; quote the wording you were given and never translate it back into an internal one. The user-facing name for the strongest of the three is "Premise unproven": always say that and never say killed, dead, or rejected. It is not a verdict on the idea. Every other score answers a conditional question (how well would this work IF the premise holds), so a premise-unproven idea can legitimately hold the highest score in the list; that combination means promising but unproven and argues for a cheap test, never that the idea is bad. No verdict mutates any stored score: the idea keeps its rank and stays selectable, and only the automatic recommendation is withheld from it and moved to the strongest idea that came through the review intact. Sources: red_team_review, apply_red_team_downgrade, and choose_auto_pick.',
