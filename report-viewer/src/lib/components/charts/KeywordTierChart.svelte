<script lang="ts">
	...
	let { tier0Count, tier1Count, tier2Count, tier3Count, tier4Count, class: className = '' }: Props =
		$props();

	// Tier configuration
	const tierConfig = [
		{ tier: 0, label: 'Quick Wins', color: 'var(--viz-cat-1)', description: 'Low competition, high value' },
		{ tier: 1, label: 'Growth', color: 'var(--viz-cat-2)', description: 'Building momentum' },
		{ tier: 2, label: 'Competitive', color: 'var(--viz-cat-3)', description: 'High competition targets' },
		{ tier: 3, label: 'Long-term', color: 'var(--viz-cat-4)', description: 'Future opportunities' },
		{ tier: 4, label: 'Monitor', color: 'var(--viz-cat-5)', description: 'Watch for changes' }
	];

	const counts = $derived([tier0Count, tier1Count, tier2Count, tier3Count, tier4Count]);
	...
	// Scales
	const xScale = $derived(scaleLinear().domain([0, maxCount]).range([0, innerWidth]));
		...
		<svg {width} {height} viewBox="0 0 {width} {height}" class="tier-chart-svg">
			<g transform="translate({margin.left}, {margin.top})">
				...
				{#each chartData as item, i}
					...
					<!-- Tier label -->
					<text x={-10} y={barY + barHeight / 2} class="tier-label" text-anchor="end" dominant-baseline="middle">
						{item.label}
					</text>
		...
		{#if hoveredBar}
			<div class="tier-tooltip">
				<div class="tooltip-header">
					<span class="tooltip-tier" style="background: {hoveredBar.color}">Tier {hoveredBar.tier}</span>
				...
				<div class="tooltip-stats">
					...
					<span class="tooltip-pct">{hoveredBar.percentage.toFixed(1)}% of total</span>
				</div>
			</div>
		{/if}
