<script lang="ts">
	...
	function getSpreadX(point: (typeof chartData)[0], index: number): number {
		...
		const spreadStep = chartData.length > 1 ? spreadRange / (chartData.length - 1) : 0;
	...
	<div class="matrix-container">
		<svg {width} {height} viewBox="0 0 {width} {height}" class="matrix-svg">
			...
			<g transform="translate({margin.left}, {margin.top})">
				...
				<!-- Quadrant labels -->
				<text x={innerWidth * 0.75} y={20} class="quadrant-label success">Best Opportunity</text>
				<text x={innerWidth * 0.25} y={20} class="quadrant-label error">Validation Risk</text>
				<text x={innerWidth * 0.75} y={innerHeight - 10} class="quadrant-label info">Nice to Have</text>
				<text x={innerWidth * 0.25} y={innerHeight - 10} class="quadrant-label muted">Deprioritize</text>

				<!-- Grid lines -->
				<line x1={innerWidth / 2} y1={0} x2={innerWidth / 2} y2={innerHeight} class="grid-line" />
				<line x1={0} y1={innerHeight / 2} x2={innerWidth} y2={innerHeight / 2} class="grid-line" />
				...
				<text x={innerWidth / 2} y={innerHeight + 25} class="axis-tick">50%</text>
				<text x={innerWidth} y={innerHeight + 25} class="axis-tick">100%</text>
				<text x={innerWidth / 2} y={innerHeight + 42} class="axis-label">Willingness to Pay</text>

				<!-- Y-axis labels -->
				<text x={-15} y={innerHeight} class="axis-tick" text-anchor="end">0%</text>
				<text x={-15} y={innerHeight / 2} class="axis-tick" text-anchor="end">50%</text>
		...
		{#if hoveredPoint}
			...
			>
				...
				<div class="tooltip-meta">
					...
					<span class="tooltip-opportunity opportunity-{hoveredPoint.opportunity}">
