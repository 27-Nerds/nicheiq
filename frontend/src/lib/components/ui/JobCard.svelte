<script lang="ts">
  ...
  const isQueued = $derived(['PENDING', 'QUEUED'].includes(job.status.toUpperCase()));
  ...
  const borderClass = $derived(
    ...
    isQueued ? 'border-l-secondary' :
    isCompleted ? 'border-l-success' :
    isFailed ? (isQualityGate ? 'border-l-warning' : 'border-l-error') :
    'border-l-border'
  );
  ...
  const dotClass = $derived(
    ...
    isFailed ? (isQualityGate ? 'bg-warning' : 'bg-error') :
    'bg-text-muted'
  );
  ...
  function getHumanReadableError(job: Job): { summary: string; suggestion: string; isQualityGate?: boolean } {
    ...
    if (error.includes('cannot be empty') || error.includes('niche description')) {
      return { summary: 'Invalid niche', suggestion: 'Please provide a more descriptive niche.' };
    }
    if (error.includes('no social content') || error.includes('no reddit') || error.includes('no twitter')) {
      return { summary: 'No discussions found', suggestion: 'Try a broader or more popular niche topic.' };
    }
    if (error.includes('cancelled') || error.includes('canceled')) {
      return { summary: 'Cancelled', suggestion: 'You cancelled this research.' };
    }
    if (error.includes('insufficient') && error.includes('credit')) {
      return { summary: 'Insufficient credits', suggestion: 'Purchase more credits to continue researching.' };
    }
    if (error.includes('timeout') || error.includes('timed out')) {
      return { summary: 'Research took too long', suggestion: 'Try with a more specific niche.' };
    }
    if (error.includes('no results') || error.includes('not found') || error.includes('no data')) {
      return { summary: 'No data found for this niche', suggestion: 'Try a different or broader niche.' };
    }
    if (error.includes('quality gate') || error.includes('confidence') || error.includes('threshold')) {
      return { summary: 'Quality check failed', suggestion: "The research didn't meet quality standards. Your credit was refunded." };
    }
    if (error.includes('requires') && error.includes('stage')) {
      return { summary: 'Pipeline error', suggestion: 'An internal error occurred. Your credit was refunded.' };
    }
    if (error.includes('dataforseo')) {
      return { summary: 'SEO data unavailable', suggestion: 'External SEO service issue. Try again later.' };
    }
    if (error.includes('rate limit') || error.includes('quota') || error.includes('429')) {
      return { summary: 'Service temporarily busy', suggestion: 'Wait a few minutes and try again.' };
    }
    if (error.includes('400') || error.includes('invalid_request') || error.includes('unsupported_parameter')) {
      return { summary: 'Configuration issue', suggestion: 'This is on our end. Your credit was refunded automatically.' };
    }
    if (error.includes('401') || error.includes('403') || error.includes('authentication') || error.includes('api key')) {
      return { summary: 'Service connection issue', suggestion: 'This is on our end. Try again later.' };
    }
    if (error.includes('500') || error.includes('502') || error.includes('503') || error.includes('server')) {
      return { summary: 'Server error', suggestion: 'Our systems are having issues. Try again later.' };
    }

    return { summary: 'Research failed', suggestion: 'Try again or contact support if the issue persists.' };
  }
...
>
  ...
  {:else if isQueued}
    ...
    <div class="flex items-center justify-between gap-4">
      ...
      <span class="text-xs px-2 py-0.5 bg-secondary/10 text-secondary rounded-full font-medium shrink-0">
        {queuePositionText}
      </span>
    </div>
    {@render projectTypeBadges()}
  ...
  {:else if isCompleted}
    ...
    <div class="flex items-center justify-between gap-4">
      ...
      <span class="text-xs text-text-muted shrink-0" title={formatDate(job.completedAt || job.createdAt)}>
  ...
  {:else if isFailed}
    ...
    <div class="flex items-center justify-between gap-4">
      ...
      <span class="text-xs text-text-muted shrink-0" title={formatDate(job.createdAt)}>
