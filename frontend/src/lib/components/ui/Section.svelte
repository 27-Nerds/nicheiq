<script lang="ts">
	...
	const badgeVariants: Record<SectionVariant, 'muted' | 'success' | 'warning' | 'accent'> = {
		default: 'muted',
		success: 'success',
		warning: 'warning',
		accent: 'accent'
...
>
	{#if isExpandable}
		...
		<button class="section-header-row expandable-trigger" onclick={() => (expanded = !expanded)}>
...
<style>
	...
	.section-success {
		...
		--section-bg: linear-gradient(135deg, rgba(34, 197, 94, 0.05) 0%, transparent 40%);
	}

	.section-warning {
		--section-border-color: rgba(239, 68, 68, 0.3);
		--section-icon-color: var(--color-error);
		--section-bg: linear-gradient(135deg, rgba(239, 68, 68, 0.05) 0%, transparent 40%);
	}

	.section-accent {
		--section-border-color: rgba(229, 90, 40, 0.3);
		--section-icon-color: var(--color-accent);
		--section-bg: linear-gradient(135deg, rgba(229, 90, 40, 0.05) 0%, transparent 40%);
	}
