<script lang="ts">
	...
	let { painPoints, analytics, solution, topPainPointsSummary }: Props = $props();
	...
	const opportunityOptions = $derived([
		...
		{ value: 'medium', label: 'Medium', count: painPoints.filter(p => p.opportunity_level === 'medium').length },
		{ value: 'low', label: 'Low', count: painPoints.filter(p => p.opportunity_level === 'low').length }
	]);

	const platforms = $derived([...new Set(painPoints.flatMap(p => p.source_platforms || []))]);
	const platformOptions = $derived(platforms.map(p => ({ value: p, label: p })));

	// Filtered pain points for analysis tab
	const filteredPainPoints = $derived(
		painPoints.filter(p => {
			if (selectedOpportunity && p.opportunity_level !== selectedOpportunity) return false;
			if (selectedPlatform && !p.source_platforms?.includes(selectedPlatform)) return false;
			return true;
		})
	);
</script>
	...
	<AnimateOnScroll animation="fade-up">
		<div class="analytics-grid mb-8">
			<div class="analytics-card analytics-card-featured">
				<div class="analytics-value text-accent">{analytics.total_pain_points}</div>
	...
	{#if activeTab === 'journey'}
		...
		<div class="pain-solution-flow">
			{#each topPainPoints as painPoint, i}
				<AnimateOnScroll animation="fade-up" delay={i * 100}>
					<div class="flow-row">
						<!-- Pain Point Card -->
						<div class="pain-card-enhanced {painPoint.severity_score >= 0.7 ? 'pain-card-severity-high' : painPoint.severity_score >= 0.5 ? 'pain-card-severity-medium' : ''}">
		...
		{#if topPainPoints.some(p => p.willingness_to_pay > 0.5)}
			<AnimateOnScroll animation="fade-up" delay={600}>
				<div class="wtp-insight">
					...
					<p class="wtp-text">
						{topPainPoints.filter(p => p.willingness_to_pay > 0.5).length} of {topPainPoints.length} top pain points
						show high willingness-to-pay indicators, suggesting strong market demand for a paid solution.
					</p>
		...
		<AnimateOnScroll animation="fade-up" delay={700}>
			<div class="journey-stats">
				...
				<div class="stat-item">
					<span class="stat-value">{painPoints.filter(p => p.severity_score >= 0.7).length}</span>
		...
		<div class="pain-points-list">
			{#each filteredPainPoints as point, index}
				<AnimateOnScroll animation="fade-up" delay={index * 100}>
					<div class="pain-point-card-enhanced {point.opportunity_level === 'high' ? 'opportunity-high' : point.opportunity_level === 'medium' ? 'opportunity-medium' : ''}">
						<!-- Header -->
						<div class="pain-point-header">
							...
							<div class="pain-point-info">
								...
								<div class="pain-point-badges">
									<span class="opportunity-badge opportunity-badge-{point.opportunity_level}">
						...
						{#if point.representative_quotes && point.representative_quotes.length > 0}
							<div class="quotes-section">
								...
								>
									...
									<span>{expandedQuotes[index] ? 'Hide' : 'Show'} {point.representative_quotes.length} quotes</span>
...
<style>
	...
	.pain-point-card-enhanced.opportunity-medium {
		border-left: 3px solid var(--color-warning);
		background: linear-gradient(135deg, rgba(245, 158, 11, 0.03) 0%, transparent 30%);
	}
