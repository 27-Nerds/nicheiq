<script lang="ts">
	...
	import type { DataSourceResearchFull, DataRoadmapPhase, DataSourceProvider, EvaluatedDataSource } from '$lib/types/report';
	...
	const costTiers = $derived.by(() => {
		...
		const tiers: { label: string; users: string; cost: string; details?: string }[] = [];
		...
		if (mvpMatch) {
			tiers.push({ label: 'MVP', users: '1K users', cost: mvpMatch[1].replace('/month', '') });
		}
		if (growthMatch) {
			const growthDetails = text.match(/Growth[^;]+/i)?.[0]?.replace(/Growth[^:]*:\s*\$[\d,]+-[\d,]+\/month\s*/i, '').trim();
			tiers.push({ label: 'Growth', users: '10K users', cost: growthMatch[1].replace('/month', ''), details: growthDetails?.replace(/^\(|\)$/g, '') });
		}
		if (scaleMatch) {
			const scaleDetails = text.match(/Scale[^.]+/i)?.[0]?.replace(/Scale[^:]*:\s*\$[\d,]+-[\d,]+\+?\/month\s*/i, '').trim();
			tiers.push({ label: 'Scale', users: '100K users', cost: scaleMatch[1].replace('/month', ''), details: scaleDetails?.replace(/^\(|\)$/g, '') });
		}
	...
	const unitEconomicsNote = $derived.by(() => {
		if (!data.estimated_monthly_cost) return null;
		const match = data.estimated_monthly_cost.match(/Target unit economics:[^.]+\./i);
		return match ? match[0] : null;
	});

	// Count metrics for hero strip
	const sourceCount = $derived((data.primary_data_sources?.length || 0) + (data.fallback_sources?.length || 0));
	...
	const getPhaseColor = (phaseName: string) => {
		...
		if (name === 'mvp') return { bg: 'rgba(34, 197, 94, 0.15)', border: '#22c55e', text: '#22c55e' };
		if (name === 'growth') return { bg: 'rgba(59, 130, 246, 0.15)', border: '#3b82f6', text: '#3b82f6' };
		if (name === 'scale') return { bg: 'rgba(168, 85, 247, 0.15)', border: '#a855f7', text: '#a855f7' };
		return { bg: 'var(--color-bg-surface)', border: 'var(--color-border)', text: 'var(--color-text-muted)' };
	};
	...
	const getQualityPercent = (value: string | undefined) => {
		...
		if (lower === 'high' || lower === 'low' || lower === 'free' || lower === 'daily' || lower === 'real-time') return 90;
...
<section id="data-infrastructure" class="report-section">
	...
	<HeroStrip>
		...
		{/snippet}
		<HeroMetric value={phaseCount} label="Phases" icon={Layers} color="accent" />
		{#if partnershipCount > 0}
			<HeroMetric value={partnershipCount} label="Partnerships" icon={Globe} color="warning" />
		{/if}
		{#if riskCount > 0}
			<HeroMetric value={riskCount} label="Risks" icon={AlertTriangle} color="error" />
		{/if}
	</HeroStrip>
			...
			<div class="cost-tiers-section">
				...
				<div class="cost-tiers-grid">
					{#each costTiers as tier, i}
						...
						<div class="cost-tier-card cost-tier-card--{tier.label.toLowerCase()}">
			...
			<div class="timeline-section">
				...
				<div class="timeline-track">
					{#each data.implementation_phases as phase, i}
						...
						<div class="timeline-phase" style="--phase-color: {colors.border}; --phase-bg: {colors.bg}">
							<!-- Timeline connector -->
							...
							<div class="phase-card">
								<div class="phase-card-header">
									<div class="phase-badge-group">
										...
										<span class="phase-name" style="color: {colors.text}">{phase.phase_name}</span>
									</div>
									<div class="phase-meta">
										<span class="phase-timeline">{phase.timeline}</span>
										<span class="phase-cost">{phase.estimated_monthly_cost}</span>
									</div>
								</div>
								...
								{#if phase.fallback_strategies && phase.fallback_strategies.length > 0}
									<details class="phase-fallbacks">
										<summary class="fallbacks-summary">
											...
											<span>{phase.fallback_strategies.length} fallback strategies</span>
										</summary>
						...
						<div class="evaluated-card">
								...
								<div class="quality-matrix">
									<div class="quality-row">
										...
										<div class="quality-bar-wrap">
											<div class="quality-bar" style="--bar-fill: {getQualityPercent(source.quality_metrics.freshness)}%"></div>
										</div>
										<span class="quality-value">{source.quality_metrics.freshness ?? 'N/A'}</span>
									</div>
									<div class="quality-row">
										<span class="quality-label">Coverage</span>
										<div class="quality-bar-wrap">
											<div class="quality-bar" style="--bar-fill: {getQualityPercent(source.quality_metrics.coverage_score)}%"></div>
										</div>
										<span class="quality-value">{source.quality_metrics.coverage_score ?? 'N/A'}</span>
									</div>
									<div class="quality-row">
										<span class="quality-label">Integration</span>
										<div class="quality-bar-wrap">
											<div class="quality-bar quality-bar--inverse" style="--bar-fill: {100 - getQualityPercent(source.quality_metrics.integration_complexity)}%"></div>
										</div>
										<span class="quality-value">{source.quality_metrics.integration_complexity ?? 'N/A'}</span>
									</div>
									<div class="quality-row">
										<span class="quality-label">Cost</span>
										<div class="quality-bar-wrap">
											<div class="quality-bar" style="--bar-fill: {getQualityPercent(source.quality_metrics.cost_viability)}%"></div>
										</div>
										<span class="quality-value">{source.quality_metrics.cost_viability ?? 'N/A'}</span>
									</div>
								</div>
							{/if}
							...
							<div class="evaluated-costs">
								{#if source.mvp_cost_estimate}
									<span class="cost-tag cost-tag--mvp">MVP: {source.mvp_cost_estimate}</span>
								{/if}
								{#if source.scale_cost_estimate}
									<span class="cost-tag cost-tag--scale">Scale: {source.scale_cost_estimate}</span>
								{/if}
							</div>
						</div>
					{/each}
...
<style>
	...
	.cost-tier-card--mvp .cost-tier-badge { color: #22c55e; }
	.cost-tier-card--growth .cost-tier-badge { color: #3b82f6; }
	.cost-tier-card--scale .cost-tier-badge { color: #a855f7; }
