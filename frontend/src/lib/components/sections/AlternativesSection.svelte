	...
	<div class="space-y-6">
		{#each data as solution, index}
			<AnimateOnScroll animation="fade-up" delay={index * 100}>
				<div class="card border-l-4 border-l-accent/50 hover:border-l-accent transition-colors">
					<!-- Header -->
					<div class="flex flex-wrap items-start justify-between gap-4 mb-4">
						<div>
							<h3 class="text-xl font-semibold text-text-primary">{solution.solution_name}</h3>
							{#if solution.value_proposition}
								<p class="text-text-secondary text-sm mt-1">{solution.value_proposition}</p>
							{/if}
						</div>
						<div class="flex flex-wrap gap-2">
							...
							{#if solution.competitive_intensity}
								<Badge variant="muted" size="sm">
									<span class={getIntensityColor(solution.competitive_intensity)}>
					...
					{:else if solution.description}
						<p class="text-text-secondary text-sm mb-4">{solution.description}</p>
					{/if}
					...
					{#if solution.technical_approach}
						<div class="card-surface mb-4">
							<div class="flex items-center gap-2 mb-2">
								...
								<span class="text-sm font-medium text-text-primary">Technical Approach</span>
							</div>
							<p class="text-sm text-text-secondary leading-relaxed">{solution.technical_approach}</p>
						</div>
					{/if}
						...
						{#if solution.core_features && solution.core_features.length > 0}
							<div class="card-surface">
								<div class="flex items-center gap-2 mb-2">
									...
									<span class="text-sm font-medium text-text-primary">Core Features</span>
								</div>
								<ul class="space-y-1">
						...
						{#if solution.target_personas && solution.target_personas.length > 0}
							<div class="card-surface">
								<div class="flex items-center gap-2 mb-2">
									...
									<span class="text-sm font-medium text-text-primary">Target Users</span>
								</div>
								<div class="flex flex-wrap gap-1">
									{#each solution.target_personas as persona}
										<span class="text-xs px-2 py-0.5 rounded bg-bg-surface border border-border text-text-muted">{persona}</span>
									{/each}
								</div>
							</div>
						{/if}
						...
						{#if solution.top_competitors && solution.top_competitors.length > 0}
							<div class="card-surface">
								<div class="flex items-center gap-2 mb-2">
									...
									<span class="text-sm font-medium text-text-primary">Top Competitors</span>
								</div>
								<div class="flex flex-wrap gap-1">
									{#each solution.top_competitors as competitor}
										<span class="text-xs px-2 py-0.5 rounded bg-warning/10 border border-warning/30 text-warning">{competitor}</span>
									{/each}
								</div>
							</div>
						{/if}
						...
						{#if solution.market_gaps && solution.market_gaps.length > 0}
							<div class="card-surface">
								<div class="flex items-center gap-2 mb-2">
									...
									<span class="text-sm font-medium text-text-primary">Market Gaps Addressed</span>
								</div>
								<ul class="space-y-1">
					...
					{#if solution.estimated_development_time || solution.estimated_cac_organic || solution.pricing_model}
						<div class="flex flex-wrap gap-4 text-sm border-t border-border pt-4 mb-4">
							{#if solution.estimated_development_time}
								<div class="flex items-center gap-2">
									<Clock class="w-4 h-4 text-text-muted" />
									<span class="text-text-muted">Dev Time:</span>
									<span class="text-text-primary">{solution.estimated_development_time}</span>
								</div>
							{/if}
					...
					<div class="grid md:grid-cols-2 gap-4 border-t border-border pt-4">
						{#if solution.key_differentiator}
							<div>
								<div class="text-xs text-text-muted mb-1">Key Differentiator</div>
						...
						{#if solution.best_suited_for}
							<div>
								...
								<p class="text-sm text-text-primary">{solution.best_suited_for}</p>
							</div>
						{/if}
					</div>
					...
					{#if solution.pivot_trigger}
						<div class="mt-4 p-3 rounded bg-warning/10 border border-warning/30">
							<div class="text-xs text-warning font-medium mb-1">When to Pivot to This Solution</div>
							<p class="text-sm text-text-secondary">{solution.pivot_trigger}</p>
						</div>
					{/if}
				</div>
			</AnimateOnScroll>
