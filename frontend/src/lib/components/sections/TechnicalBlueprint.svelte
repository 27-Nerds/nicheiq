<script lang="ts">
	...
	let { solution, implementationOverview, mvpScope, userJourney, dataInfrastructureRoadmap, siteStructure, userFlows }: Props = $props();
	...
	function getPriorityVariant(priority: string): 'success' | 'warning' | 'default' {
		switch (priority) {
			case 'P0': return 'success';
	...
	function getPageTypeVariant(pageType: string): 'accent' | 'default' | 'muted' {
		switch (pageType) {
			case 'programmatic': return 'accent';
			...
			<div class="site-structure-section">
				...
				<div class="site-overview-card">
					...
					<div class="site-stats-grid">
						...
						<div class="site-stat">
							<span class="site-stat-value">{siteStructure.total_static_pages}</span>
							<span class="site-stat-label">Static Pages</span>
						</div>
						<div class="site-stat">
							<span class="site-stat-value">{siteStructure.total_programmatic_pages}</span>
					...
					{#if siteStructure.tech_stack_recommendation}
						<div class="tech-stack-callout">
							...
							<span class="tech-stack-text">{siteStructure.tech_stack_recommendation}</span>
						</div>
					{/if}
				</div>
						...
						<div class="site-section-card">
							...
							>
								...
								<div class="site-section-header-right">
									<Badge variant="muted" size="sm">{section.pages.length} pages</Badge>
										...
										<div class="site-page-row">
											<div class="site-page-info">
												<div class="site-page-name-row">
													...
													<Badge variant={getPriorityVariant(page.priority)} size="sm">{page.priority}</Badge>
													<Badge variant={getPageTypeVariant(page.page_type)} size="sm">{page.page_type}</Badge>
												</div>
