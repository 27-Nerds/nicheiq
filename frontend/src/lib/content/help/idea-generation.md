...
Scoring an idea well and an idea surviving contact with the real world aren't the same test, so a slice of the pool goes through one more round before it reaches you: an adversarial review (internally we sometimes call it a red-team pass). The slice is chosen to cover different kinds of risk rather than one leaderboard — the idea most likely to become the recommendation, plus the most *shippable* candidate (the one a single builder could realistically build and run), plus the strongest of the rest. Fresh web searches look specifically for reasons the idea might not hold up — a competitor that shows up under the category's own name rather than the idea's phrasing, a capability some other tool already gives away for free, a mechanism that handles the clean version of the problem but not the messy one most users actually have. Every finding comes with the evidence attached, and the review lands on one of three calls: the idea survives, it's weakened, or it's killed outright. If the search comes back empty — or comes back about a plainly different industry, which happens when an idea's own wording collides with someone else's vocabulary — nothing changes: we don't invent a flaw to justify the step, we don't treat silence as a clean bill of health, and we don't let a bad search masquerade as market evidence.
...
The selection screen also includes an **analyst summary** of the visible pool. It is meant to distinguish captured findings from caveats, including payability, competition, and ruled-out directions. It may recommend a next comparison or evidence check, but that guidance does not change the ranking or make the choice for you.
