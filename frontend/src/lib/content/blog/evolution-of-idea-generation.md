...
## Step 11: the reviewer of last resort

Every improvement in this post so far tightened something inside the pipeline: the critic, the caps, the lenses. But the flaws that kept slipping through weren't inside-the-pipeline flaws. They were the kind a person only catches by going and looking, which is exactly what we'd been doing by hand for every run we shipped before we trusted the system to ship on its own. Three patterns showed up again and again. An incumbent search built from the idea's own phrasing misses the competitor that sells the same thing under the category's actual name — "grant reconciliation software" turns up nothing when the vendor calls it "compliance monitoring." A capability an idea wants to charge for is often already sitting free inside a tool the buyer has open all day, a spreadsheet template, a platform feature nobody thought to search for because it isn't sold as its own product. And a mechanism that reads clean in the writeup sometimes only works for the textbook version of the problem, not the messy modal case real users actually have.

None of that is exotic. It's the same skeptical-buyer instinct a human does for free in fifteen minutes on Google. So we stopped treating it as something only a human could do and built it in: ecosystem and free-bundle probes that search in the category's own vocabulary instead of the idea's, a modal-case question added to the critique loop that asks not just "does this mechanism work" but "does it work for the common case, not just the clean one," and — for the ideas with the strongest market fit, the ones actually worth the scrutiny — a full adversarial web review. Fresh searches, an evidence-cited verdict of survives, weakened, or killed, and one guarded shot at revision: if the evidence points at a fixable flaw, a rewrite gets one attempt, and it has to beat the original through the identical scoring gauntlet — same critic, same caps — or the original stands with the flaw noted. No evidence found, no verdict changed; absence of evidence doesn't get to pose as evidence of a gap.

Two runs told us this was worth shipping. In the first, a freight-bond idea came back reshaped. The pool's original concept was a bond lookup tool, a commodity three sites already offer, and the adversarial pass flagged exactly that. The revision it triggered turned the idea into an eligibility check plus a claim-packet generator: figure out whether a shipper can even file, then produce the paperwork. We're calling it BondClaim internally (QuickPay to whoever wrote the pricing doc first — we still haven't agreed). What made the result worth writing up is that a human reviewer, working the same niche by hand the day before, had independently landed on the identical wedge. The system re-derived, unprompted, what a person had just proven out.

The second run was the opposite kind of validation. A pool topped out with a confident 0.75 — the highest-scoring idea in a fresh run, everything about it reading like a "Go." The adversarial review killed it, with a citation: the incumbent already bundles that exact capability into its own sales process, so a standalone tool would be competing with something the buyer gets for free from the vendor they're already talking to. No market-fit score survives a competitor giving the thing away as a courtesy call. That's a paid Deep Research cycle we didn't put a user through on an idea that was never going to hold up.

Both runs point at the same shift. Up to Step 10, everything in this post was in service of finding the best idea in a pool. This step is in service of something else: making sure that when the system hands you a winner, it survived being attacked, and when it can't find one, it says so with the same evidence a human reviewer would have used to say the same thing. The job was never to always produce a winner. It's to be right about whether one is there.
