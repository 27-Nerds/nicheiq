...
<div class="max-w-6xl">
  ...
  {#if showCreateForm}
    <div class="bg-bg-surface border border-border rounded-xl p-5 mb-6">
      <h3 class="text-lg font-semibold text-text-primary mb-4">New Promo Code</h3>
      {#if formError}
      ...
      <form onsubmit={(e) => { e.preventDefault(); handleCreate(); }} class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label for="code" class="block text-sm font-medium text-text-secondary mb-1">Code</label>
          <input id="code" type="text" bind:value={newCode} required
        ...
        <div>
          <label for="credits" class="block text-sm font-medium text-text-secondary mb-1">Credit Amount</label>
          <input id="credits" type="number" bind:value={newCreditAmount} required min="1"
            class="w-full px-3 py-2 bg-bg-elevated border border-border rounded-lg text-text-primary text-sm focus:outline-none focus:border-accent" />
        </div>
        <div>
          <label for="maxRedemptions" class="block text-sm font-medium text-text-secondary mb-1">Max Redemptions</label>
          <input id="maxRedemptions" type="number" bind:value={newMaxRedemptions} required min="1"
            class="w-full px-3 py-2 bg-bg-elevated border border-border rounded-lg text-text-primary text-sm focus:outline-none focus:border-accent" />
        </div>
        <div>
          <label for="expiresAt" class="block text-sm font-medium text-text-secondary mb-1">Expires At (optional)</label>
          <input id="expiresAt" type="date" bind:value={newExpiresAt}
            class="w-full px-3 py-2 bg-bg-elevated border border-border rounded-lg text-text-primary text-sm focus:outline-none focus:border-accent" />
        </div>
        <div class="sm:col-span-2">
          <label for="description" class="block text-sm font-medium text-text-secondary mb-1">Description (optional)</label>
          <input id="description" type="text" bind:value={newDescription}
            class="w-full px-3 py-2 bg-bg-elevated border border-border rounded-lg text-text-primary text-sm focus:outline-none focus:border-accent"
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="border-b border-border bg-bg-elevated/50">
              <th class="text-left py-3 px-4 text-text-muted font-medium">Code</th>
              <th class="text-right py-3 px-4 text-text-muted font-medium">Credits</th>
              <th class="text-right py-3 px-4 text-text-muted font-medium">Uses</th>
              <th class="text-left py-3 px-4 text-text-muted font-medium">Expires</th>
          ...
          <tbody>
            {#each data.promoData.promoCodes as promo}
              <tr class="border-b border-border/50">
                <td class="py-3 px-4 font-mono font-semibold text-text-primary">{promo.code}</td>
                <td class="py-3 px-4 text-right text-text-primary">{promo.creditAmount}</td>
                <td class="py-3 px-4 text-right text-text-secondary">
                  {promo.currentUses} / {promo.maxRedemptions}
                </td>
                <td class="py-3 px-4 text-text-muted">{formatDate(promo.expiresAt)}</td>
                <td class="py-3 px-4">
                  <Badge variant={promo.isActive ? 'success' : 'muted'} size="sm">
