<script lang="ts">
  ...
  // FAQ-only mini-editor for catalog ideas. Mirrors the FAQ section of the
  // category SEO editor but does NOT expose other SEO fields (per O8 — full
  // idea SEO management is deferred to v2 if editorial demand emerges).
  ...
  // Resync key combines idea id + faqJsonMeta.updatedAt so a save (manual or
  // generator-driven) bumps the key and the local `faq` array refreshes from
  // the freshly-loaded data. Same-id-only guard would leave stale empty
  // state until a hard reload.
  let syncedKey = $state<string | null>(null);
  let faq = $state<FaqEntry[]>([]);

  $effect(() => {
    const key = `${data.idea.id}:${data.idea.faqJsonMeta?.updatedAt ?? ""}`;
    if (syncedKey === key) return;
    syncedKey = key;
    faq = Array.isArray(data.idea.faqJson)
      ? data.idea.faqJson.map((f) => ({ q: f.q, a: f.a }))
      : [];
  });
...
<svelte:head>
  <title>FAQ · {data.idea.solutionName} | Admin | NicheIQ</title>
</svelte:head>

<div class="seo-editor max-w-3xl">
  <header class="mb-6">
    <a href="/admin/catalog?tab=curate&type=ideas" class="back-link">
      <ArrowLeft class="w-3.5 h-3.5" />
      Back to ideas
    </a>
    <h1 class="text-2xl font-bold text-text-primary mt-3">
      FAQ · {data.idea.headline ?? data.idea.solutionName}
    </h1>
    <p class="text-sm text-text-muted mt-1">
      <span class="font-mono text-xs">
        {data.idea.category.parent?.name ?? "—"} / {data.idea.category.name}
      </span>
      <span class="mx-1.5">·</span>
      <span class="font-mono text-xs">codename: {data.idea.solutionName}</span>
    </p>
    <p class="text-xs text-text-muted mt-2">
      FAQs on idea pages are anchored on the niche name (not the codename) for
      AI-search citation. Public pages refresh within ~15&nbsp;min after save (CDN cache).
    </p>
  </header>
  ...
  <div class="field">
    ...
    <div class="faq-generator-row">
      <FaqGenerator
        ...
        meta={data.idea.faqJsonMeta ?? null}
        onSaved={async () => {
          await invalidateAll();
        }}
      />
    ...
    {#if faq.length === 0}
      <p class="empty-faq">
        ...
        a set from this idea's data, or add entries manually below.
      </p>
    {/if}
    <div class="faq-list">
  ...
  <div class="actions">
    <a href="/admin/catalog?tab=curate&type=ideas" class="cancel-link">Cancel</a>
