<script lang="ts">
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (!res.ok) {
        feedback = { type: 'error', message: result.error || 'Failed to save setting' };
        return;
      }

      feedback = { type: 'success', message: 'Sample report URL saved successfully' };
      await invalidateAll();
      userUrl = null;
    } catch {
  ...
  async function handleClear() {
    ...
    try {
      ...
      if (!res.ok) {
        feedback = { type: 'error', message: result.error || 'Failed to clear setting' };
        return;
      }
...
<div class="max-w-2xl">
  ...
  <div class="bg-bg-surface border border-border rounded-xl p-5">
    ...
    <p class="text-sm text-text-muted mb-4">
      Configure which shared report is displayed on the public <code class="text-xs bg-bg-elevated px-1.5 py-0.5 rounded">/sample-report</code> page.
    </p>

    {#if feedback}
      <div class="flex items-center gap-2 text-sm mb-4 p-2.5 rounded-lg {feedback.type === 'success' ? 'bg-success/10 text-success' : 'bg-error/10 text-error'}">
    ...
    <form onsubmit={(e) => { e.preventDefault(); handleSave(); }}>
      <label for="sample-report-url" class="block text-sm font-medium text-text-secondary mb-1">
        Share URL
      </label>
      <input
        ...
        type="text"
        value={currentUrl}
        oninput={(e) => { userUrl = (e.target as HTMLInputElement).value; }}
      ...
      <p class="text-xs text-text-muted mt-1.5">
        Paste the share URL path from a completed report (e.g. <code class="bg-bg-elevated px-1 py-0.5 rounded">/shared/abc123</code>).
      </p>
